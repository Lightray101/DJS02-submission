...
/**
 * Main class to manage the Podcast Application
 */
class PodcastApp {
  /**
  ...
  /**
   * Renders the podcast previews on the main page.
  ...
  renderPodcasts(podcastsToRender) {
    ...
    podcastsToRender.forEach((podcast) => {
      ...
      card.innerHTML = `
        ...
        podcast.title
      }" class="podcast-card__image" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
          ...
          <div class="podcast-card__content">
            ...
            <div class="podcast-card__genres">
              ${podcast.genres
                .map(
                  (id) =>
                    `<span class="podcast-card__genre-tag">${this.genres.get(
                      id
                    )}</span>`
                )
                .join("")}
        ...
        `;
      card.addEventListener("click", () => this.openModal(podcast.id));
